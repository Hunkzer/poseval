...
class COCO:
    """Class for the COCO Pose Model as used in openpose

    Contains joints decoded as integer numbers. Map integers to names using self.name[int]
    ...
    def __init__(self):
        ...
        self._name = {}
        self._name[self.right_ankle] = "right_ankle"
        self._name[self.right_knee] = "right_knee"
        self._name[self.right_hip] = "right_hip"
        self._name[self.right_shoulder] = "right_shoulder"
        self._name[self.right_elbow] = "right_elbow"
        self._name[self.right_wrist] = "right_wrist"
        self._name[self.left_ankle] = "left_ankle"
        self._name[self.left_knee] = "left_knee"
        self._name[self.left_hip] = "left_hip"
        self._name[self.left_shoulder] = "left_shoulder"
        self._name[self.left_elbow] = "left_elbow"
        self._name[self.left_wrist] = "left_wrist"
        self._name[self.neck] = "neck"
        self._name[self.nose] = "nose"

        self._name[self.left_hip] = "left_hip"
        self._name[self.right_hip] = "right_hip"
        self._name[self.left_eye] = "left_eye"
        self._name[self.right_eye] = "right_eye"
        self._name[self.left_ear] = "left_ear"
        self._name[self.right_ear] = "right_ear"

        array = []
        for i in range(self.count):
            array.append(self._name[i])
        self._kp_names = array

        self._color = {}
        self._color[self.neck] = "pink"
        self._color[self.nose] = "crimson"
        self._color[self.left_eye] = "magenta"
        self._color[self.right_eye] = "magenta"
        self._color[self.left_ear] = "purple"
        self._color[self.right_ear] = "purple"
        self._color[self.right_shoulder] = "blue"
        self._color[self.left_shoulder] = "blue"
        self._color[self.right_elbow] = "cornflowerblue"
        self._color[self.left_elbow] = "cornflowerblue"
        self._color[self.right_wrist] = "slategrey"
        self._color[self.left_wrist] = "slategrey"
        self._color[self.right_hip] = "peru"
        self._color[self.left_hip] = "peru"
        self._color[self.right_knee] = "green"
        self._color[self.left_knee] = "green"
        self._color[self.right_ankle] = "orange"
        self._color[self.left_ankle] = "orange"

        self._pairs = [[1, 8],  # Body
                       [1, 11],
                       [1, 2],
                       [1, 5],
                       [2, 3],
    ...
    @property
    def name(self):
        """Contains a mapping from integer (body part index) to string (name of body part)
    ...
    @property
    def kp_names(self):
        """Get names of all keypoints returned in an array.

        Returns:
            array: contains names of keypoints
    ...
    @property
    def pairs(self):
        """Get keypoint pairs returned as 2D array.

        Returns:
            2D array. Contains arrays of length 2 containing the respective pairs as integers
    ...
    @property
    def color(self):
        """Contains a mapping from integer (body part index) to matplotlib named color (string)
...
class BODY25:
    """Class for the BODY_25 Pose Model of openpose.
    ...
    def __init__(self):
        ...
        self._name[self.left_hip] = "left_hip"
        self._name[self.right_hip] = "right_hip"
        self._name[self.mid_hip] = "mid_hip"
        self._name[self.left_big_toe] = "left_big_toe"
        self._name[self.right_big_toe] = "right_big_toe"
        self._name[self.left_small_toe] = "left_small_toe"
        self._name[self.right_small_toe] = "right_small_toe"
        self._name[self.left_heel] = "left_heel"
        self._name[self.right_heel] = "right_heel"
        self._name[self.left_eye] = "left_eye"
        self._name[self.right_eye] = "right_eye"
        self._name[self.left_ear] = "left_ear"
        self._name[self.right_ear] = "right_ear"
        # background doesn't need to have a name
...
def getCum(vals):
    ...
    cum = []
    n = -1
    cum += [(vals[[coco.neck, coco.nose,
                   coco.right_eye, coco.left_eye,
                   coco.right_ear, coco.left_ear], 0].mean())]
    cum += [(vals[[coco.right_shoulder, coco.left_shoulder], 0].mean())]
    cum += [(vals[[coco.right_elbow, coco.left_elbow], 0].mean())]
    cum += [(vals[[coco.right_wrist, coco.left_wrist], 0].mean())]
    cum += [(vals[[coco.right_hip, coco.left_hip], 0].mean())]
    cum += [(vals[[coco.right_knee, coco.left_knee], 0].mean())]
    cum += [(vals[[coco.right_ankle, coco.left_ankle], 0].mean())]
    for i in range(coco.count, len(vals)):
        cum += [vals[i, 0]]
    return cum
    ...
    for imgidx in range(len(gtFrames)):
        ...
        if len(prFrames[imgidx]["annorect"]) and len(gtFrames[imgidx]["annorect"]):
            ...
            for ridxGT in range(len(gtFrames[imgidx]["annorect"])):
                ...
                # print("x1:", rectGT["x1"][0])
                # print("x2:", rectGT["x2"][0])
                # print("y1:", rectGT["y1"][0])
                # print("y2:", rectGT["y2"][0])
                ...
                for ridxPr in range(len(prFrames[imgidx]["annorect"])):
                    ...
                    # iterate over all possible body joints
                    # print("pointsGT = ", pointsGT)
                    # print("pointsPr = ", pointsPr)
                    for i in range(nJoints):
                        # GT joint
                        ppGT = getPointGTbyID(pointsGT, i)
                        # predicted joint
            ...
            # print("assign predicted poses to GT poses")
            for ridxPr in range(hasPr.shape[0]):
                if ridxPr in prToGT:  # pose matches to GT
                    ...
                    # print(ridxPr, " in ", prToGT, " ?")
                    # print("pose matches not to GT")
                    s = score[ridxPr, :]
            ...
            if not len(gtFrames[imgidx]["annorect"]):
                ...
                # print("No GT available. All predictions are false positives")
                for ridxPr in range(hasPr.shape[0]):
                    s = score[ridxPr, :]
